...
if __name__ == '__main__':
    ...
    # batch wise images x
    z = feature_extractor(x) # output now has the features corresponding to input x

        
    # archetype analysis to get k dim archetypes from all the images
    aa_no = 3
    out_aa = spams.archetypalAnalysis(z, p = aa_no, stepsAS=3) # 128 x aa_no
    out_aa = torch.from_numpy(out_aa).to(device)
    
    # get dictionary B    
    embedding = nn.Embedding(output.shape[1], aa_no)  #  2048 x aa_no
    embedding.weight.data = torch.Tensor(out_aa)
    ...
    def __init__(self):
        ...
        self.model = nn.Sequential(
            *block(2048, 1024, normalize=False),
            *block(1024, 512),
            *block(512, 256),
            *block(256, aa_no),
            nn.Tanh()
    ...
    gen_z = generator(z)  # produces alphas
    
    # reconstruction loss
    loss_0 = np.linalg.norm( z - B@gen_z) + lam * np.linalg.norm(gen_z, 1)
    
    # 
    loss_1 = discriminator(out_aa)
    
    # doubt: out_aa and z has different dimension
    loss_2 = discriminator(z) - 1
    
    #
    loss_3 = discriminator(B@gen_z)
